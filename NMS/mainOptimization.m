...
model = 'nms_model_modified2019aPush';
...
for i = 4:6
    ...
    for j = 1:length(distAmp)
        j
%         waitbar(counter/(length(disturbances)*length(distAmp)),h)
        % The 7th disturbance is a push
        counter = counter + 1;
        disturbances = zeros(7,1);
...
% Boundaries
C2  = linspecer(7,'red');
h(1) = plot([0,100],[0,0],'color',C2(3,:),'LineWidth',1);
h(2) = plot([0,100],[5,5],'color',C2(3,:),'LineWidth',1);
